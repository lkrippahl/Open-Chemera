...
type
  TFileBuffer=class
    ...
    procedure WriteIntegers(const Ints:TOCIntegers);
    procedure ReadIntegers(out Ints:TOCIntegers);
    function GetIntegers:TOCIntegers;
    procedure WriteDoubles(const Ds:TOCDoubles);overload;
    procedure ReadDoubles(out Ds:TOCDoubles);overload;
    procedure WriteSingles(const Ds:TOCSingles);
    procedure ReadSingles(out Ds:TOCSingles);
    procedure WriteStrings(const ss:TOCStrings);
    procedure ReadStrings(out ss:TOCStrings);
...
procedure TFileBuffer.ReadIntegers(out Ints:TOCIntegers);
...
procedure TFileBuffer.WriteIntegers(const Ints:TOCIntegers);
...
procedure TFileBuffer.ReadDoubles(out Ds: TOCDoubles);
var f:Integer;
...
procedure TFileBuffer.ReadSingles(out Ds: TOCSingles);
var f:Integer;
...
procedure TFileBuffer.WriteDoubles(const Ds: TOCDoubles);
var f:Integer;
...
procedure TFileBuffer.WriteSingles(const Ds: TOCSingles);
var f:Integer;
...
procedure TFileBuffer.ReadStrings(out ss: TOCStrings);
var f:Integer;

begin
  SetLength(ss,GetInteger);
  for f:=0 to High(ss) do ReadString(ss[f]);
end;

procedure TFileBuffer.WriteStrings(const ss: TOCStrings);
var f:Integer;
...
function TFileBuffer.GetIntegers: TOCIntegers;
  var f:Integer;
begin
